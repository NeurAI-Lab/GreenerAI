...
def test(args):
    ...
    model.head.spatial_path.addcoords_spatial = AddCoords([512, 1024], coord_scales=[1.0, 2.0])
    model.backbone.addcoords = AddCoords([512, 1024], coord_scales=[1.0, 2.0])
    ...
    for i, (image, dst) in enumerate(tbar):
        ...
        if args.eval:
            with torch.no_grad():
                ...
                image = functional.interpolate(image, scale_factor=0.5, align_corners=False, mode='bilinear')
                dst = functional.interpolate(dst.unsqueeze(0), scale_factor=0.5, mode='nearest').squeeze(0).long()
