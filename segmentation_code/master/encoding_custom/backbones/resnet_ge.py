...
from encoding_custom.nn.Gather_Excite_Layer import GEBlock
...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.context_GE_block0 = GEBlock(64, [256, 256], learned=ge_trained)
        self.context_GE_block1 = GEBlock(128, [128, 128], learned=ge_trained)
        self.context_GE_block2 = GEBlock(256, [64, 64], learned=ge_trained)
        self.context_GE_block3 = GEBlock(512, [32, 32], learned=ge_trained)
    ...
    def forward(self, x):
        ...
        c1 = self.layer1(x)
        c1 = self.context_GE_block0(c1)
        c2 = self.layer2(c1)
        c2 = self.context_GE_block1(c2)
        c3 = self.layer3(c2)
        c3 = self.context_GE_block2(c3)
        c4 = self.layer4(c3)
        c4 = self.context_GE_block3(c4)

        return c1, c2, c3, c4


def resnet18_ge(pretrained=False, root="", **kwargs):
...
def resnet34_ge(pretrained=False, root="", **kwargs):
...
def resnet50_ge(pretrained=False, root="~/.encoding/models", **kwargs):
...
def resnet101_ge(pretrained=False, root="~/.encoding/models", **kwargs):
...
def resnet152_ge(pretrained=False, root="~/.encoding/models", **kwargs):
