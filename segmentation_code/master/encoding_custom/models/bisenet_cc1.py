...
class SpatialPath(nn.Module):
    def __init__(self, in_planes, out_planes, norm_layer=nn.BatchNorm2d):
        ...
        inner_channel = 64
        self.addcoords_spatial = AddCoords([512, 512])

        self.conv_7x7_cc = ConvBnRelu(
            in_planes+3,
            inner_channel,
            7,
            2,
    ...
    def forward(self, x):
        x = self.addcoords_spatial(x)
        x = self.conv_7x7_cc(x)
        x = self.conv_3x3_1(x)
        x = self.conv_3x3_2(x)
...
class BiSeNet_wrapper_cc1(BaseNet):
