    ...
    def __init__(
        ...
        if backbone.startswith("resnet"):
            if "antialiased" in backbone:
                self.pretrained = {
                    "resnet18_antialiased": backbones.resnet18_antialiased,
                    "resnet34_antialiased": backbones.resnet34_antialiased,
                    "resnet50_antialiased": backbones.resnet50_antialiased(),
                    "resnet101_antialiased": backbones.resnet101_antialiased(),
                    "resnet152_antialiased": backbones.resnet152_antialiased(),
                }[backbone](
                    pretrained=pretrained,
                    norm_layer=norm_layer,
                    filter_size=anti_aliased,
                )
            else:
                self.pretrained = {
    ...
    def base_forward(self, x):
        if "inplace" in self.backbone:
            ...
            if hasattr(self.pretrained, "layer0"):
                ...
                """if self.pretrained.anti_aliased is not None:
                    x = self.pretrained.downsample(x)"""
