...
class BiSeNet(nn.Module):
    def __init__(
        ...
        self.spatial_path = SpatialPath(3, 128, norm_layer, anti_aliased=anti_aliased)
...
class SpatialPath(nn.Module):
    def __init__(self, in_planes, out_planes, norm_layer=nn.BatchNorm2d, anti_aliased=None):
...
class BiSeNet_wrapper(BaseNet):
    def __init__(self, num_classes, backbone, aux=False, se_loss=False, anti_aliased=None, **kwargs):
        super().__init__(num_classes, backbone, aux, se_loss, anti_aliased=anti_aliased, **kwargs)
        self.head = BiSeNet(num_classes, self.pretrained.base_inchannels, anti_aliased=anti_aliased)
...
def get_bisenet(
    dataset="", backbone="", pretrained=False, anti_aliased=None, root="~/.encoding/models", **kwargs
):
