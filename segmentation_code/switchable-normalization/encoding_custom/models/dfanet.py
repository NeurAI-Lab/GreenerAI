...
class enc(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stage):
        ...
        for i in range(rep_nums - 1):
            rep.append(
                Block(out_channels, out_channels, stride=1, start_with_relu=True)
            )
...
class DfaNet(BaseNet):
    def __init__(
        ...
        **kwargs
        ...
        self.head = xceptionAx3(num_classes)
...
class xceptionAx3(nn.Module):
    ...
    def forward(self, x):
        ...
        up_fca1 = F.interpolate(
            stage_fca1, stage_enc2a.size()[2:], mode="bilinear", align_corners=False
        )
        ...
        up_fca2 = F.interpolate(
            stage_fca2, stage_enc2b.size()[2:], mode="bilinear", align_corners=False
        )
        # stage c
        ...
        x2_up = F.interpolate(x2, x1.size()[2:], mode="bilinear", align_corners=False)
        x3 = self.enc2c_to_decoder_dim_reduction(stage_enc2c)
        x3_up = F.interpolate(x3, x1.size()[2:], mode="bilinear", align_corners=False)
        # print(x3.size())
        x_up = x1 + x2_up + x3_up
        ...
        # print(x_fca1.size())
        x_fca1_up = F.interpolate(
            x_fca1, x1.size()[2:], mode="bilinear", align_corners=False
        )
        x_fca2 = self.fca2_to_decoder_dim_reduction(stage_fca2)
        # print(x_fca2.size())
        x_fca2_up = F.interpolate(
            x_fca2, x1.size()[2:], mode="bilinear", align_corners=False
        )
        x_fca3 = self.fca3_to_decoder_dim_reduction(stage_fca3)

        # print(x_fca3.size())
        x_fca3_up = F.interpolate(
            x_fca3, x1.size()[2:], mode="bilinear", align_corners=False
        )
...
def get_dfanet(
    ...
    **kwargs
    ...
    model = DfaNet(datasets[dataset.lower()].NUM_CLASS)
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(
                get_model_file(
                    "deeplab_%s_%s" % (backbone, acronyms[dataset]), root=root
                )
            )
        )
    return model
