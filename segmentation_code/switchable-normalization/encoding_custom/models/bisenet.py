...
class BiSeNet_wrapper(BaseNet):
    def __init__(self, num_classes, backbone, aux=False, se_loss=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super().__init__(num_classes, backbone, aux, se_loss, norm_layer=nn.BatchNorm2d, **kwargs)
        self.head = BiSeNet(num_classes, self.pretrained.base_inchannels, norm_layer=norm_layer)
...
def get_bisenet(
    dataset="", backbone="", pretrained=False, norm_layer=nn.BatchNorm2d, root="~/.encoding/models", **kwargs
):
