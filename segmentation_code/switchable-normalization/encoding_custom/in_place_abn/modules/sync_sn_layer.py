...
class SyncSNFunc(Function):
    ...
    def backward(ctx, grad_outdata):
        if grad_outdata.is_cuda:
            ...
            in_data, scale_data, x_hat, mean, var, mean_in, var_in, mean_ln, var_ln, mean_bn, var_bn, mean_weight, var_weight = (
                ctx.saved_tensors
            )
            ...
            meanDiff = (
                -1 / (var.view(N, C) + ctx.eps).sqrt() * torch.sum(x_hatDiff, [2, 3])
            )

            varDiff = (
                -0.5 / (var.view(N, C) + ctx.eps) * torch.sum(x_hatDiff * x_hat, [2, 3])
            )
            ...
            term21 = (
                var_weight[0]
                * 2
                * (in_data.view(N, C, H, W) - mean_in.view(N, C, 1, 1))
                / (H * W)
                * varDiff.view(N, C, 1, 1)
            )
            term22 = (
                var_weight[1]
                * 2
                * (in_data.view(N, C, H, W) - mean_ln.view(N, 1, 1, 1))
                / (C * H * W)
                * torch.sum(varDiff, [1]).view(N, 1, 1, 1)
            )
            ...
            term23 = (
                var_weight[2]
                * 2
                * (in_data.view(N, C, H, W) - mean_bn.view(1, C, 1, 1))
                / (N * H * W)
                * term23_tmp
                / dist.get_world_size()
            )
            ...
            term32 = (
                mean_weight[1] * torch.sum(meanDiff, [1]).view(N, 1, 1, 1) / C / H / W
            )
            ...
            mean_weight_Diff[0] = mean_weight[0] * (
                mw1_diff
                - mean_weight[0] * mw1_diff
                - mean_weight[1] * mw2_diff
                - mean_weight[2] * mw3_diff
            )
            mean_weight_Diff[1] = mean_weight[1] * (
                mw2_diff
                - mean_weight[0] * mw1_diff
                - mean_weight[1] * mw2_diff
                - mean_weight[2] * mw3_diff
            )
            mean_weight_Diff[2] = mean_weight[2] * (
                mw3_diff
                - mean_weight[0] * mw1_diff
                - mean_weight[1] * mw2_diff
                - mean_weight[2] * mw3_diff
            )
            var_weight_Diff[0] = var_weight[0] * (
                vw1_diff
                - var_weight[0] * vw1_diff
                - var_weight[1] * vw2_diff
                - var_weight[2] * vw3_diff
            )
            var_weight_Diff[1] = var_weight[1] * (
                vw2_diff
                - var_weight[0] * vw1_diff
                - var_weight[1] * vw2_diff
                - var_weight[2] * vw3_diff
            )
            var_weight_Diff[2] = var_weight[2] * (
                vw3_diff
                - var_weight[0] * vw1_diff
                - var_weight[1] * vw2_diff
                - var_weight[2] * vw3_diff
